          ...
          <div style={{ width:'100%', aspectRatio:'1/1', overflow:'hidden', background:'#c8c0b0', position:'relative' }}>
            {memeUrl
              ? <img
                  src={memeUrl}
                  alt=""
                  className="p3-develop"
                  onError={(e) => { e.target.style.display = 'none'; }}
                  style={{ width:'100%', height:'100%', objectFit:'cover', objectPosition:'center top', display:'block' }}
                />
              : null
            }
            {!memeUrl && <div style={{ width:'100%', height:'100%', background:'#c8c0b0' }} />}
            <div style={{
              position:'absolute', inset:0, pointerEvents:'none',
              ...
              {['grayscale(0.1) sepia(0.02)', 'none', 'grayscale(0.3) contrast(1.15)'].map((filter, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', gap: 'clamp(4px, 0.8vw, 6px)' }}>
                    <div style={{ flex: 1, aspectRatio: '1/1', overflow: 'hidden', background: '#d1d1d1' }}>
                      {userImgUrl
                        ? <img src={userImgUrl} alt="" className="photo-strip-img" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 20%', filter }} />
                        : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', background: '#e5e5e5' }}>📸</div>}
                    </div>
                    <div style={{ flex: 1, aspectRatio: '1/1', overflow: 'hidden', background: '#2a2520', position: 'relative' }}>
                      {currentMemeUrl
                        ? <img
                            src={currentMemeUrl}
                            alt=""
                            ...
                            onError={(e) => {
                              console.error('[Meme load failed]', currentMemeUrl);
                          ...
                          />
                        : null}
                      <div style={{ display: currentMemeUrl ? 'none' : 'flex', width: '100%', height: '100%', position: 'absolute', inset: 0, alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>🐾</div>
                    </div>
                  </div>
                )
              })}
...
function ResultPage({ result, onScanAgain, onBackHome }) {
  ...
  useEffect(() => {
    ...
    const processUrl = (url, addTimestamp = true) => {
      if (!url) return '';
      if (url.startsWith('data:')) return url;
      // Force HTTPS — iOS Safari blocks mixed content (http image on https page)
      const secure = url.replace(/^http:\/\//i, 'https://');
      if (!addTimestamp) return secure;
      const separator = secure.includes('?') ? '&' : '?';
      return `${secure}${separator}t=${Date.now()}`;
    }
    ...
    // meme URLs: ใช้ HTTPS แต่ไม่เติม timestamp — ป้องกัน invalidate signed URL / Supabase token
    const memes = rawMemes.map(m => processUrl(m, false));
